...
**Antes de empezar, completa Retos 0-2. Asegúrate de tener las tablas Silver listas y los datasets JSON cargados en Fabric.**
...
2️⃣ Aplica **agregaciones** y define **relaciones clave** entre tablas por `id` (de products.json) o un campo equivalente en creditScore.json (ej. `productId` si existe).  
3️⃣ Si los datasets no comparten una clave, combina manualmente en un notebook (ej. usando PySpark para unir por `id`).  
4️⃣ Asegúrate de que todas las tablas necesarias para análisis estén consolidadas (ej. `products_gold` de products.json y `creditScore_gold` de creditScore.json).  
...
1️⃣ En **Power BI o Microsoft Fabric**, diseña el **modelo semántico Gold** incluyendo:  
   - 🔹 **Dimensiones:** `Brand`, `Category`, `perfil_producto` (derivada: ej. categoriza por Price > 100 como 'Premium'), `Availability`.  
   - 📏 **Medidas clave:**  
...
3️⃣ Si tienes múltiples tablas (productos, creditScore), crea las relaciones por **id** o campo equivalente.  
...
4️⃣ Vincula con tablas como `products_gold` y `creditScore_gold` (creadas en Paso 1).  
...
**Valida que las medidas funcionen importando una muestra de los JSON en Fabric.**
